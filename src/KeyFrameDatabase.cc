...
void KeyFrameDatabase::AddMap(KeyFrameDatabase *srcDB)
{
	...
	std::vector<list<KeyFrame*> > mTmpInvFile = srcDB->GetInvertedFile();

	set<KeyFrame*> sKf;
	for( size_t i = 0; i < mTmpInvFile.size(); i++ )
	{
		const list<KeyFrame*> &lTmpEntries = mTmpInvFile[i];
		for( KeyFrame * pKF : lTmpEntries )
		{
			mvInvertedFile[i].push_back(pKF);
			sKf.insert(pKF);
		}
	}

	for( KeyFrame *pKF : sKf )
		pKF->SetKeyFrameDatabase(this);
}

std::vector<list<KeyFrame*> > KeyFrameDatabase::GetInvertedFile()
{
...
void KeyFrameDatabase::add(KeyFrame *pKF)
{
    ...
    for(fbow::fBow::const_iterator vit= pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit!=vend; vit++) {
        ...
        if (vit->first > mvInvertedFile.size())
            mvInvertedFile.resize(vit->first * 1.25);
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        {
            nscores++;
            float si = fbow::fBow::score(F->mBowVec,pKFi->mBowVec);
            pKFi->mRelocScore=si;
            lScoreAndMatch.push_back(make_pair(si,pKFi));
        }
    }
