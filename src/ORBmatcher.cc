...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(!pMP->GetContext(nAgentId).mbTrackInView)
            continue;

        if(pMP->isBad())
            continue;

        const int &nPredictedLevel = pMP->GetContext(nAgentId).mnTrackScaleLevel;

        // The size of the window will depend on the viewing direction
        float r = RadiusByViewingCos(pMP->GetContext(nAgentId).mTrackViewCos);

        if(bFactor)
            r*=th;

        const vector<size_t> vIndices =
                F.GetFeaturesInArea(pMP->GetContext(nAgentId).mTrackProjX, pMP->GetContext(nAgentId).mTrackProjY,
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            if(F.mvuRight[idx]>0)
            {
                const float er = fabs(pMP->GetContext(nAgentId).mTrackProjXR-F.mvuRight[idx]);
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    if(!bStereo1 && !bStereo2)
                    {
                        ...
                        auto bad_code_factor = (pKF2->mvScaleFactors.empty()) ? 1.0 : pKF2->mvScaleFactors[kp2.octave];
                        if(distex*distex+distey*distey<100*bad_code_factor)
                            continue;
                    }
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    boost::unique_lock<boost::shared_mutex> lockKF(pKF->mFeaturesSharedMutex);
