								 ...
								 int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
	...
	std::set<long unsigned int> snKFIds;
	...
	for (size_t i = 0; i < vpMP.size(); i++)
	{
		...
		for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++)
		{
			...
			if (pKF->isBad() || snKFIds.count(pKF->mnId) == 0)
				continue;
...
int Optimizer::PoseOptimization(Frame *pFrame, Map *pMap)
{
	...
	//	std::cerr << "Robot: " << pFrame->GetAgentId() << " Time: " << ttrack * 1000.0 << " # " <<  nInitialCorrespondences << std::endl;
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap)
{
	...
	pKF->GetContext(nAgentId).mnBALocalForKF = pKF->mnId;
	...
	for (int i = 0, iend = vNeighKFs.size(); i < iend; i++)
	{
		...
		if (!pKFi->isBad())
		{
			pKFi->GetContext(nAgentId).mnBALocalForKF = pKF->mnId;
	...
	for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
	{
		...
		for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend; vit++)
		{
			...
			if (pMP)
				if (!pMP->isBad())
				{
					if (pMP->GetContext(nAgentId).mnBALocalForKF != pKF->mnId)
					{
						pMP->GetContext(nAgentId).mnBALocalForKF = pKF->mnId;
	...
	for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
	{
		...
		for (map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
		{
			...
			if (pKFi->GetContext(nAgentId).mnBALocalForKF != pKF->mnId && pKFi->GetContext(nAgentId).mnBAFixedForKF != pKF->mnId)
			{
				if (!pKFi->isBad())
				{
					pKFi->GetContext(nAgentId).mnBAFixedForKF = pKF->mnId;
	...
	std::set<unsigned long int> sKeyFrameIds;
	...
	for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
	{
		...
		for (map<KeyFrame *, size_t>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
		{
			...
			if (!sKeyFrameIds.count(pKFi->mnId))
				continue;
