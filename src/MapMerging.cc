...
void MapMerging::MergeMaps()
{
	...
	std::cerr << "Overlap detected: Merging Map " << srcMapId << " into Map " << dstMapId << endl;
	...
	for (MapHolder *pSrcMapHoldder : vpSrcMapHoldder)
	{
		...
		while (!pSrcMapHoldder->pLocalMapper->isStopped() || !pSrcMapHoldder->pLoopCloser->isFinished() || !pSrcMapHoldder->pMapDrawer->isStopped())
		{
			usleep(1000);
		}
	}
	...
	for (MapHolder *pDstMapHoldder : vpDstMapHoldder)
	{
		...
		while (!pDstMapHoldder->pLocalMapper->isStopped() || !pDstMapHoldder->pLoopCloser->isStopped() || !pDstMapHoldder->pMapDrawer->isStopped())
		{
			usleep(1000);
		}
	}
...
{
	std::cerr << "Starting Global Bundle Adjustment" << endl;
	...
	// We need to propagate the correction through the spanning tree
	{
		...
		std::cerr << "Global Bundle Adjustment finished" << endl;
		std::cerr << "Updating map ..." << endl;
		...
		std::cerr << "Keyframes corrected" << std::endl;
