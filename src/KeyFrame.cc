...
KeyFrame::KeyFrame(long unsigned int nId, Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, cv::Mat img) : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                                                                                                       ...
                                                                                                       mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb / 2), mpMap(pMap), mbFirst(false)
{
    mnId = nId;
    mnAgentId = F.GetAgentId();
    ...
    if (!img.empty())
    {
        bgr.resize(N);
        for (int i = 0; i < N; i++)
            bgr[i] = img.at<cv::Vec3b>(mvKeys[i].pt);
    }

    mvAgentContext.resize(MultiAgentInfo::gnMaxRobotId + 1);
}

void KeyFrame::SetFirst()
{
    mbFirst = true;
}

bool KeyFrame::IsFirst()
{
    return mbFirst;
}
...
int KeyFrame::GetAgentId()
{
    return mnAgentId;
}

void KeyFrame::AddParent(KeyFrame *pKF)
{
    unique_lock<mutex> lockCon(mMutexEssential);
    mpParent = pKF;
    mbFirst = false;
    pKF->AddChild(this);
...
{
    boost::unique_lock<boost::shared_mutex> lock(mMutexFeatures);
...
{
    boost::shared_lock<boost::shared_mutex> lock(mMutexFeatures);
    set<MapPoint *> s;
...
void KeyFrame::SetBadFlag()
{
    ...
    {
        unique_lock<mutex> lock(mMutexEssential);
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    {
        boost::shared_lock<boost::shared_mutex> lock(mMutexFeatures);
        boost::shared_lock<boost::shared_mutex> lock2(mMutexPose);
