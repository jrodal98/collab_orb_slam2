...
bool LoopClosing::DetectLoop()
{
	...
	//If the map contains less than 5 KF have passed from last loop detection
	if (mbRecentLoopClosed && mnKeyFrameCounter < 5)
	{
		mnKeyFrameCounter++;
...
void LoopClosing::CorrectLoop()
{
	...
	int nMapId = mpMap->GetMapId();
	std::vector<MapHolder *> vpMapHolder = mpMapDatabase->GetMapHolderByMapId(nMapId);

	// Stop everything
	for (MapHolder *pMapHoldder : vpMapHolder)
	{
		pMapHoldder->pLocalMapper->RequestStop();
	}

	// Stop map merging
	mpMapDatabase->mpMapMerger->RequestStop();
	...
	for (MapHolder *pMapHoldder : vpMapHolder)
	{
		while (!pMapHoldder->pLocalMapper->isStopped())
		{
			usleep(1000);
		}
		pMapHoldder->pTracker->Pause();
	}
	...
	for (MapHolder *pMapHoldder : vpMapHolder)
	{
		pMapHoldder->pLocalMapper->Release();
		pMapHoldder->pTracker->Resume();
	}

	// Continue map merging
	mpMapDatabase->mpMapMerger->Release();
...
{
	cerr << "Starting Global Bundle Adjustment" << endl;
	...
	// We need to propagate the correction through the spanning tree
	{
		...
		if (!mbStopGBA)
		{
			cerr << "Global Bundle Adjustment finished" << endl;
...
void LoopClosing::RequestStop()
{
	...
	std::cerr << "Loop Closing Request Stop - LoopClosing::RequestStop()" << std::endl;
}
...
void LoopClosing::Release()
{
	...
	std::cerr << "Loop Closing Release" << std::endl;
}
