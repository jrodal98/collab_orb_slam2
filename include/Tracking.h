...
public:
    Tracking(System *pSys, ChosenVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap,
             KeyFrameDatabase *pKFDB, SLAMConfig *pSlamConfig, const int sensor, int nRobotId);

    cv::Mat TrackMonocular(const cv::Mat &descriptions, const std::vector<cv::KeyPoint> &keypoints, const int rows, const int cols);
...
public:
    ...
    std::mutex mMutexTrackedPoints;

    boost::shared_mutex mMutexTracking;
    std::vector<double> mvtTrack;
    std::vector<double> mvtTrackLocalMap;
    std::vector<double> mvtTrackReferenceKeyFrame;
    std::vector<double> mvtTrackWithMotionModel;
